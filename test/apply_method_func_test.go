...
func TestApplyMethodFunc(t *testing.T) {
	...
	var s *fake.Slice
	Convey(
		"TestApplyMethodFunc", t, func() {
			Convey(
				"for succ", func() {
					err := slice.Add(1)
					So(err, ShouldEqual, nil)
					patches := ApplyMethodFunc(
						s, "Add", func(_ int) error {
							return nil
						},
					)
					defer patches.Reset()
			...
			Convey(
				"for variadic method", func() {
					slice = fake.NewSlice()
					count := slice.Append(1, 2, 3)
					So(count, ShouldEqual, 3)
					patches := ApplyMethodFunc(
						s, "Append", func(_ ...int) int {
							return 0
						},
					)
					defer patches.Reset()
